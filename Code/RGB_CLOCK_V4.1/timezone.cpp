...
const TimezoneEntry timezoneDatabase[] PROGMEM = {
    {"Africa/Abidjan", "GMT0"},
    {"Africa/Accra", "GMT0"},
...
const int TIMEZONE_COUNT = sizeof(timezoneDatabase) / sizeof(timezoneDatabase[0]);

String getPosixString(const char* zoneName) {
    for (int i = 0; i < TIMEZONE_COUNT; i++) {
        TimezoneEntry entry;
        memcpy_P(&entry, &timezoneDatabase[i], sizeof(TimezoneEntry));
        
        if (strcmp(zoneName, entry.zoneName) == 0) {
            return String(entry.posixString);
        }
    }
    return "";
}
...
void printAllTimezones() {
    Serial.println("Available Timezones (" + String(TIMEZONE_COUNT) + " total):");
